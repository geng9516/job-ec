...
//                System.out.println("ファイルが存在しません。");
    ...
    //
    public static void read4(List<String> itemCodeList) {
        ...
        //循环要要拷贝的照片名文件夹
        for (String s : itemCodeList) {
            checkFileExitst4(file.getAbsolutePath(), s);
        }
    }

    //判断itemCode的文件价存在
    public static void checkFileExitst4(String filePath, String fileName) {
        //把文件路径的文件价抽象化
        File file = new File(filePath);
        if (file.exists()) {
            File[] files = file.listFiles();
...
//                System.out.println("ファイルが存在しません。");
                ...
                for (File file1 : files) {
                    ...
                    if (file1.isDirectory()) {
                        ...
                        if (".DS_Store".equals(file1.getName())) {
                            ...
                            //调用拷贝方法 传入filepath
                            copyItemPhoto4(file1.getAbsolutePath(), file1.getName());
                            //不一致时递归
                        } else {
                            checkFileExitst4(file1.getAbsolutePath(), fileName);
                        }
                    }
                }
            }
    ...
    //照片拷贝方法
    public static void copyItemPhoto4(String filePath, String folderName) {
